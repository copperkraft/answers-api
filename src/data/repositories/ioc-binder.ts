...
export const bindRepositories = (container: Container, storage: DataStorage<AnswersAppSchema>): void => {
    container.bind<AnswerRepository>('AnswerRepository').toConstantValue(new AnswerSequelizeRepository(storage.models.Answer.model, storage.models));
    container.bind<UserRepository>('UserRepository').toConstantValue(new UserSequelizeRepository(storage.models.User.model, storage.models));
    container.bind<TagRepository>('TagRepository').toConstantValue(new TagSequelizeRepository(storage.models.Tag.model, storage.models));
};